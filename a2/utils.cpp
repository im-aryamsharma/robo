...
SensorData update_sensors()
{
	...
	data.avg_distance = get_array_average(distances, DISTANCE_LOOKBACK);
	data.avg_angle = get_array_average(angles, GYRO_LOOKBACK);
...
int get_speed(int d)
{
	int result = (MAX_SPEED - MIN_SPEED) / pow((MAX_DISTANCE - MIN_DISTANCE), 2) * pow((d - MIN_DISTANCE), 2) + MIN_SPEED;
	return constrain(result, MIN_SPEED, MAX_SPEED);
}
...
{
	int current_heading = getAngle();
	int error = target_heading - current_heading;

  if (abs(error) <= GYRO_EPSILON) return;

	while(abs(error) > GYRO_EPSILON)
	{
    updateGyroAngle();
		current_heading = getAngle();
		...
		int speed = constrain(abs(error) * 5, MIN_SPEED, MAX_SPEED);

    print("gyro", "speed", speed);

		if (error > 0) { move_motors(-speed, speed); }
		if (error < 0) { move_motors(speed, -speed); }
